...
if (isset($uuid) && isset($frame_speed)) {
	...
	if ($reponse->rowCount() > 0) {
		while ($donnees = $reponse->fetch()) {
			...
			if ($title) {
				$updateRequest = 'UPDATE shoots SET title = :title, thumbnail = :thumbnail WHERE uuid = :uuid';
				$req = $bdd->prepare($updateRequest);
				$req->execute(array(
					'uuid' => $uuid,
					'title' => $title,
					'thumbnail' => $thumbnail
				...
				$updateRequest = 'UPDATE shoots SET thumbnail = :thumbnail WHERE uuid = :uuid';
				$query = $bdd->prepare($updateRequest);
				$query->execute(array(
					'uuid' => $uuid,
					'thumbnail' => $thumbnail
				));
